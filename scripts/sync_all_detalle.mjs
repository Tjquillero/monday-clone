...
async function main() {
    ...
    for (let r = 0; r < rows.length; r++) {
        ...
        // Col 3 = Code, Col 4 = Desc, Col 5 = Unit, Col 7 = Unit Price
        ...
        // Basic validation that it's an item row
        if (code && /^[0-9]+(?:\.[0-9]+)+$/.test(code) && desc && !isNaN(price)) {
            codeToData.set(code, {
                code,
                name: `${code} ${desc}`, // Ensure it has the code prefix or just use desc? Let's use code + desc
                rawName: desc,
                price,
                unit
            });
    ...
    console.log(`Parsed ${codeToData.size} valid items from DETALLE DE ACTA.`);
    ...
    for (const item of items) {
        ...
        if (extractedCode && codeToData.has(extractedCode)) {
            ...
            // Normalize current name to compare
            ...
            if (needsUpdate) {
                ...
                console.log(`Updating ${extractedCode}: Price ${item.values.unit_price} -> ${excelData.price}, Name length ${item.name.length} -> ${newName.length}`);
                updatedCount++;
            }
        }
    }
