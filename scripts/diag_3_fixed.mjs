...
async function main() {
    ...
    console.log(`=== MASTER ITEMS MATCHING 3.* ===`);
    const masters = itemsFound.filter(i => !i.group_id);
    for (const m of masters) {
        console.log(`ID: ${m.id} | Code: ${m.values?.code} | Name: ${m.name.substring(0, 50)}`);
    }
    
    console.log(`\n=== ALL ITEMS MATCHING 3.* (${itemsFound.length} total) ===`);
    const groupedByCode = {};
    ...
    if (ids.length > 0) {
        const { data: details, error } = await supabase.from('financial_acta_details').select('id, item_id, quantity').in('item_id', ids);
        if (error) {
            console.log("Error fetching details:", error.message);
        } else {
            console.log(`\nFound ${details?.length || 0} financial details linked to exact 3.1/3.10/3.12 items.`);
            let counts = {};
            if (details) {
