...
**Estado (cierre del Incremento 5):** los pasos (1) y (4) ya están implementados y probados — (1) es `poa_zone_mappings` (migración + RLS), y (4) es la función `import_poa_version()`, cuya especificación completa (firma, tipos, invariantes, errores esperables, ejemplo) vive en [`import-poa-version-contract.md`](./import-poa-version-contract.md), no en este documento. El paso (2) (parser + validación) también está implementado en `src/lib/poaImport/`. Pendientes: el paso (3) (UI de resolución de mapeos) y el servicio orquestador en TypeScript que conecta (2) con (4) — construir el JSON exacto que `import_poa_version()` espera y generar el `import_operation_id`.
