...
**Estado (backend del Incremento 5: completo).** Los pasos (1), (2) y (4) están implementados y probados: (1) es `poa_zone_mappings` (migración + RLS); (2) es el parser y la validación en `src/lib/poaImport/` (capas 1-3); (4) es la función `import_poa_version()`, cuya especificación completa (firma, tipos, invariantes, errores esperables, ejemplo) vive en [`import-poa-version-contract.md`](./import-poa-version-contract.md), no en este documento. El servicio orquestador en TypeScript que conecta (2) con (4) — `importPoaService`, `src/lib/poaImport/service/` — también está completo (5 commits, 66/66 tests); su flujo está documentado en [`import-poa-orchestrator-flow.md`](./import-poa-orchestrator-flow.md). **Único paso pendiente: (3), la UI de resolución de mapeos de zona**, más la UI de importación que consume `importPoaService` — trabajo de aplicación, no de backend. El backend no debería necesitar cambios para construir ninguna de las dos, salvo que aparezca un defecto real.
