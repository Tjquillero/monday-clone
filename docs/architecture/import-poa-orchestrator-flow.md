...
validateParsedPoa()                src/lib/poaImport/validate.ts
      ...
      ├── frecuencia pendiente ──► ambiguousFrequencyActivities (motivo: different_values | mixed_null_and_value)
...
- Los tres motivos de `blocked` se acumulan a lo largo del flujo (no se corta en el primero) — un mismo intento puede tener zonas sin mapear Y actividades con frecuencia pendiente Y errores de validación a la vez, y `createBlockedResult()` los recibe juntos. Una celda `FREC.` vacía en TODAS las zonas de una actividad ya no cuenta como pendiente (ADR-0005) — se resuelve a `frecuencia = null` sin bloquear.
...
- [`ADR-0005`](../adr/ADR-0005-poa-frecuencia-ausente.md) — por qué una `FREC.` vacía en todas las zonas de una actividad no bloquea la importación.
- [`poa-import-observability.md`](./poa-import-observability.md) — propuesta (no implementada) de métricas sobre este flujo: éxitos, bloqueos por categoría, `sqlState`, duración.
