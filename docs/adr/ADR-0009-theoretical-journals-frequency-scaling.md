...
**Nota de actualización (2026-07-21)**: apareció posible evidencia relevante para el "Criterio para revisar esta decisión" (ver abajo) — un documento operativo real (`COSTOS GENERALES (V2).xlsx`, de Operaciones) calcula "CANT JORNALES MES" con la fórmula algebraicamente idéntica a la que este ADR reemplazó (`qty × 25 / (rendimiento × frecuencia)`). Esto NO reabre el ADR todavía — falta confirmar con Operaciones qué representa exactamente esa columna del Excel (¿la misma pregunta que resuelve `theoreticalJournals`, o un presupuesto/dimensionamiento distinto?). Investigación abierta: `docs/operacion/investigaciones/costos/INV-0002-formula-jornales-vs-adr0009.md`.
