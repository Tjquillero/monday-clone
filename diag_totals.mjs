...
async function check() {
    const boardId = 'ff77409f-848a-48a1-8f29-6e6e3cc8cde3'; // ID from previous logs check
    const { data: groups } = await supabase.from('groups').select('*').eq('board_id', boardId).order('position');
    console.log('Groups on Board:', groups.map(g => g.title));
    ...
    for (const g of groups) {
        ...
        console.log(`Site: ${g.title} | Budget: ${bTotal} | Executed: ${eTotal} | Items: ${items.length}`);
    }
}
