...
import { Group, Item, ActivityTemplate, Column } from '@/types/monday';
import { motion } from 'framer-motion';
import { Calendar as CalendarIcon, TrendingUp, Plus, Calculator, X, Camera, Check, Wand2, User, Trash2, MoreHorizontal, Loader2, CheckCircle2 } from 'lucide-react';
...
export default function ExecutionView({ 
  ...
  const isRestricted = userRole === 'supervisor' || userRole === 'encargado';
    ...
    <div className="flex flex-col h-full bg-white border border-gray-200 rounded-xl shadow-sm font-sans">
            ...
            <div className="w-12 h-12 bg-slate-800 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-slate-200">
             ...
             <div>
                ...
                <div className="flex items-center space-x-2">
                  <p className="text-sm text-slate-500">Control de ejecución diaria por sitio y actividad</p>
                </div>
             </div>
         </div>
      ...
      <div className="overflow-x-auto flex-1 custom-scrollbar">
        <div className="min-w-[1500px]">
          {/* Grid Header */}
          <div className="grid grid-cols-[60px_200px_350px_150px_80px_60px_80px_80px_80px_80px_repeat(7,110px)_110px_100px_60px] bg-white sticky top-0 z-30 shadow-sm border-b border-slate-200">
             <HeaderCell label="ITE" />
             <HeaderCell label="SITIO" />
             ...
             {days.map(day => (
               <div key={day.id} className="border-r border-slate-100 p-2 flex flex-col items-center justify-center bg-slate-50/30">
          ...
          <div className="bg-white">
            {groups.map((group) => (
              <div key={group.id}>
                {/* Group Header Row */}
                <div className="bg-slate-50/50 border-b border-slate-100 flex items-center px-4 py-2 justify-between">
                   <div className="flex items-center space-x-2">
                   ...
                   {!isRestricted && (
                    <button 
                      ...
                      className="flex items-center text-[10px] font-bold text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 px-2 py-1 rounded transition-all border border-transparent hover:border-emerald-100"
                    >
                ...
                {group.items.map((item: Item, idx: number) => {
                  const renderRow = (currentItem: Item, level: number = 0, indexStr: string) => {
                      ...
                      const cantTotal = parseFloat(currentItem.values['cant']) || 0;
                      const rend = parseFloat(currentItem.values['rend']) || 0;
                      const frec = parseFloat(currentItem.values['frec']) || 25; 
                      const unit = currentItem.values['unit'] || 'M2';
                      ...
                      return (
                        <div key={currentItem.id}>
                            <div 
                              className={`grid grid-cols-[60px_200px_350px_150px_80px_60px_80px_80px_80px_80px_repeat(7,110px)_110px_100px_60px] border-b border-slate-50 hover:bg-slate-50/50 transition-colors h-[48px] items-center ${isSubItem ? 'bg-slate-50/20' : ''} ${hasIncidents ? 'bg-rose-50/10' : ''}`}
                            >
                               <Cell text={indexStr} gray={isSubItem} />
                               <Cell text={isSubItem ? '' : group.title} weight="bold" color="#64748b" />
                               ...
                               <div className="h-full border-r border-slate-50 flex items-center justify-center">
                                ...
                                <div className="h-full border-r border-slate-50 flex items-center justify-center">
                                   <EditableCell 
                                     type="number"
                                     value={currentItem.values['frec'] || '25'}
                                     disabled={isRestricted}
                               ...
                               {days.map(day => {
                                 ...
                                 return (
                                   <div key={day.id} className={`h-full border-r border-slate-50 flex items-center px-1 transition-colors ${isDone ? 'bg-emerald-50/50' : ''}`}>
                                  ...
                                  {!isRestricted && (
                                   <>
                                      ...
                                      <button 
                                        ...
                                        className="p-1.5 hover:bg-rose-50 text-slate-300 hover:text-rose-500 rounded-lg transition-all opacity-0 group-hover/actions:opacity-100 cursor-pointer z-[60]"
                                        title="Eliminar"
                                      >
                                         <Trash2 className="w-4 h-4" />
                                      </button>
                            ...
                            {/* Recursive Render */}
                            {currentItem.subItems && currentItem.subItems.map((sub, sIdx) => renderRow(sub, level + 1, `${indexStr}.${sIdx + 1}`))}
                        </div>
                      );
                  };
